...
export function LoadingContext({ children }: { children: ReactNode }) {
  ...
  const setLoading = useCallback((show: boolean) => {
    ...
    if (show) loadingNodeRef.current.classList.remove("hidden");
    else loadingNodeRef.current.classList.add("hidden");
  }, []);

  const isLoading = useCallback(
    () => Boolean(loadingNodeRef.current?.classList.contains("hidden")),
    []
  );
